...
export default function AnniversaryList() {
  ...
  const deletetem = (index, seniordetails) => {
    setSeniorname(seniordetails);
    setAlert(true);
    var deletePost = [];
  ...
  console.log(SeniorAnniversaryList);

  return (
    <div className="">
      {/* {showAlert &&
        <AppToast showAleart={showAlert} icon="mgc_check_circle_fill" message={${seniorName} post deleted successfully} />}
      {enableempty ? <SkeletonPage /> : seniorAnniversaryList.length === 0 || seniorAnniversaryList === "[]" ? (
        ...
        <Box sx={{ flexGrow: 1 }}>
          <Grid container>
            ...
            <Grid item key={idx} xs={12} sm={6} md={4} lg={3}>
              <Senior anniversaryitemssenior={ele} itemIndex={idx} key={idx} deleteSenior={deletetem}></Senior>
            </Grid>
             ))} 
          </Grid>
